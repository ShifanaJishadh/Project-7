...
def test_model_inputs(model_inputs):
    with tf.Graph().as_default():
        ...
        assert target_sequence_length.op.type == 'Placeholder',             'Target Sequence Length is not a Placeholder.'
        assert max_target_sequence_length.op.type == 'Max',             'Max Target Sequence Length is not a Placeholder.'
        assert source_sequence_length.op.type == 'Placeholder',             'Source Sequence Length is not a Placeholder.'

        # Check name
        assert input_data.name == 'input:0',            'Input has bad name.  Found name {}'.format(input_data.name)
        assert target_sequence_length.name == 'target_sequence_length:0',            'Target Sequence Length has bad name.  Found name {}'.format(target_sequence_length.name)
        assert source_sequence_length.name == 'source_sequence_length:0',            'Source Sequence Length has bad name.  Found name {}'.format(source_sequence_length.name)
...
def test_decoding_layer(decoding_layer):
    ...
    with tf.Graph().as_default():
        ...
        assert isinstance(train_decoder_output, tf.contrib.seq2seq.BasicDecoderOutput),            'Found wrong type: {}'.format(type(train_decoder_output))
        assert isinstance(infer_logits_output, tf.contrib.seq2seq.BasicDecoderOutput),            'Found wrong type: {}'.format(type(infer_logits_output))

        assert train_decoder_output.rnn_output.get_shape().as_list() == [batch_size, None, vocab_size],             'Wrong shape returned.  Found {}'.format(train_decoder_output.rnn_output.get_shape())
        assert infer_logits_output.sample_id.get_shape().as_list() == [batch_size, None],              'Wrong shape returned.  Found {}'.format(infer_logits_output.sample_id.get_shape())
